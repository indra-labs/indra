...
func NewKeyBook(_ context.Context, store ds.Datastore,
	_ Options) (*dsKeyBook, error) {
...
func (kb *dsKeyBook) PubKey(p peer.ID) ic.PubKey {
	...
	if value, err := kb.ds.Get(context.TODO(), key); err == nil {
		...
		if err != nil {
			log.Errorf("error when unmarshalling pubkey "+
		...
		case peer.ErrNoPublicKey:
			return nil
		default:
			log.Errorf("error when extracting pubkey from "+
				"peer ID for peer %s: %s\n", p.Pretty(), err)
			return nil
		}
		pkb, err := ic.MarshalPublicKey(pk)
		if err != nil {
			log.Errorf("error when turning extracted pubkey "+
				"into bytes for peer %s: %s\n", p.Pretty(), err)
			return nil
		}
		if err := kb.ds.Put(context.TODO(), key, pkb); err != nil {
			log.Errorf("error when adding extracted pubkey "+
				"to peerstore for peer %s: %s\n", p.Pretty(), err)
			return nil
		}
	} else {
		log.Errorf("error when fetching pubkey from datastore "+
			"for peer %s: %s\n", p.Pretty(), err)
	}
...
func (kb *dsKeyBook) AddPubKey(p peer.ID, pk ic.PubKey) error {
	...
	if err != nil {
		log.Errorf("error while converting pubkey byte string "+
			"for peer %s: %s\n", p.Pretty(), err)
		return err
	}
	if err := kb.ds.Put(context.TODO(), peerToKey(p, pubSuffix),
		val); err != nil {
...
func (kb *dsKeyBook) AddPrivKey(p peer.ID, sk ic.PrivKey) error {
	...
	if err != nil {
		log.Errorf("error while converting privkey "+
			"byte string for peer %s: %s\n", p.Pretty(), err)
		return err
	}
	if err := kb.ds.Put(context.TODO(), peerToKey(p, privSuffix),
		val); err != nil {

		log.Errorf("error while updating privkey in "+
			"datastore for peer %s: %s\n", p.Pretty(), err)
	}
	return err
}
...
func peerToKey(p peer.ID, suffix ds.Key) ds.Key {
	return kbBase.ChildString(base32.RawStdEncoding.
		EncodeToString([]byte(p))).Child(suffix)
}
