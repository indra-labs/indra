...
var (
	...
	// the deterministic nonce for the EC-Schnorr-INDRAv0 scheme.  This ensures
	// the same nonce is not generated for the same message and key as for other
	// signing algorithms such as ECDSA.
	//
	// It is equal to SHA256([]byte("EC-Schnorr-INDRAv0")).
	rfc6979ExtraDataV0 = [32]byte{
		0x33, 0x48, 0x56, 0xef, 0xdb, 0x2f, 0x66, 0x3a,
		0xa1, 0x0d, 0xe9, 0xf5, 0xc0, 0x97, 0xff, 0xad,
		0xee, 0x5d, 0xf3, 0xe1, 0x32, 0x6a, 0x0d, 0x6d,
		0xa4, 0x25, 0x83, 0x39, 0x7b, 0x8a, 0xd4, 0xb0,
	}
)
...
// ParseSignature parses a signature according to the EC-Schnorr-INDRAv0
...
func schnorrVerify(sig *Signature, hash []byte, pubKey *secp256k1.PublicKey) error {
	// The algorithm for producing a EC-Schnorr-INDRAv0 signature is described in
	...
	// 3. Fail if r >= p
	// 4. Fail if s >= n
	// 5. e = SHA256(r || m) (Ensure r is padded to 32 bytes)
	// 6. Fail if e >= n
	// 7. R = s*G + e*Q
	...
	// Step 5.
	//
	// e = SHA256(r || m) (Ensure r is padded to 32 bytes)
...
// schnorrSign generates an EC-Schnorr-INDRAv0 signature over the secp256k1 curve
...
func schnorrSign(privKey, nonce *secp256k1.ModNScalar, hash []byte) (*Signature, error) {
	...
	// 7. e = SHA256(r || m) (Ensure r is padded to 32 bytes)
...
// Sign generates an EC-Schnorr-INDRAv0 signature over the secp256k1 curve for the
