...
`EC-Schnorr-DCRv0` that is described herein. (TODO: give name variant to
indicate this is for Indra, and uses only SHA256 hashes)  The signatures and
...
* They are provably secure with weaker assumptions than the best known security
  proofs for ECDSA
	* Specifically Schnorr signatures are provably secure under SUF-CMA (
	  Strong
	  Existential Unforgeability under Chosen Message Attack) in the ROM (
	  Random
	  Oracle Model) which guarantees that as long as the hash function
	  behaves
	  ideally, the only way to break Schnorr signatures is by solving the
	  ECDLP
...
* ~~Uses BLAKE-256 with 14 rounds for the hash function to calculate
  challenge `e`~~
* Uses SHA256 to calculate the challenge `e'
...
* The exact serialization of the data used to create the challenge `e` varies,
  but
...
Decred results in the choice of `e = SHA256(R.x || m)` for the 
challenge with
additional restrictions on `R` to ensure verifiers can reconstruct the full
point: TODO: changing this to use SHA256 to eliminate inclusion of 
unnecessary other hash functions.
...
* ~~The BLAKE-256 hash function with 14 rounds is already in widespread use and
  satisfies all requirements needed for the hash function~~ TODO: SHA256 is also
  perfectly fine for this purpose and eliminates a second hash function 
  where one is enough. Nothing special about Blake256 that precludes using 
  SHA256 in its place.
...
7. e = ~~BLAKE-256~~SHA256(r || m) (Ensure r is padded to 32 bytes)
...
3. Fail if r >= p
4. Fail if s >= n
5. e = ~~BLAKE-256~~SHA256(r || m) (Ensure r is padded to 32 bytes)
6. Fail if e >= n
7. R = s*G + e*Q
...
* `EC-Schnorr-v2.0` and `EC-Schnorr-v2.10` specified in BSI TR-03111, Versions
  2.0
