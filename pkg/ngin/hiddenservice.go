...
func MakeHiddenService(header slice.Bytes, in *Intro, client *SessionData,
...
func (ng *Engine) SendHiddenService(header slice.Bytes, id nonce.ID, key *prv.Key, expiry time.Time,
	...
	o := MakeHiddenService(header, in, target, c, ng.KeySet)
	log.D.F("%s sending out hidden service onion %s",
		ng.GetLocalNodeAddressString(),
		color.Yellow.Sprint(target.AddrPort.String()))
